Vagrant.configure("2") do |config|
  ...
  config.trigger.before [:up, :provision] do |trigger|
    ...
    trigger.run = {path: "provisioner/scripts/create-ssh-keys.sh"}
  end
  ...
  config.vm.define "dev01" do |dev01|
    ...
    #  ansible.playbook = "provisioner/playbooks/deploy-assets-manager.yml"
    #end  
  ...
  config.vm.define "dev02" do |dev02|
    ...
    config.vm.provision "shell",
      path: "provisioner/scripts/copy-ssh-keys.sh"
  ...
  config.vm.define "ops01" do |ops01|
    ...
    #config.vm.provision "shell",
    #  path: "provisioner/scripts/install-ansible-debian.sh"      
    
    ops01.vm.provision "ansible" do |ansible|
    # ansible.playbook = "provisioner/playbooks/deploy-ansible-jenkins.yml"
    # ansible.playbook = "provisioner/playbooks/deploy-lamp.yml"
      ansible.playbook = "provisioner/playbooks/deploy-test.yml"
    end    
